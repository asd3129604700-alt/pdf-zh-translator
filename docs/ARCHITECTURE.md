...
### 碎块：合并相邻条目（修「贴不全 + 有的字小」）

用户的原话：**"还是存在贴不全还有部分字小的问题，但是同一批有的是完全正常的让我有点搞不懂了，
是切太多块了吗，那要不要一个字附近有字就一起切，隔得很开才单独切"**。
他的判断是对的，而且这一个原因同时解释了三个症状。

上游（视觉模型 / OCR / PDF 文字层）经常把**一整行切成好几块** —— 一个标签一块、
一个值一块，甚至一个词一块。之后排版阶段每一块都要靠"邻居边界"约束自己：

- 擦除范围被左右邻居夹住 → 原文边缘擦不掉（**贴不全**）
- 可用横向空间被切成几份 → 字号被迫缩小（**字小**）
- 上下邻居还会夹住可用高度 → 中文放不下就再缩一档

而**孤立的块没有邻居，拿到的是完整空间** —— 所以同一批里有的完全正常、有的很糟，
看起来毫无规律。

修法是 `PZUtil.mergeAdjacentItems`：把同一行上挨得近的条目并成一条。
四个判据必须同时成立（宁可少合并，也不要跨表格列乱并）：

| 判据 | 值 | 作用 |
|---|---|---|
| 纵向重叠 | ≥ 较矮那个的 55% | 确认在同一行 |
| 高度比 | ≤ 1.8 | 别把大标题和小标注并到一起 |
| 横向间距 | ≤ 较矮那个的 1.2 倍 | "挨得近"；表格列之间的空隙远大于此 |
| 横向间距 | ≥ -0.35 倍 | 大幅重叠的是重复识别，交给去重，不是相邻 |

合并后原文用空格连接、译文按语言习惯连（中文之间不加空格）。图片和 PDF 两条
流水线都做，顺序是 `去重 → 合并 → 再去重`（先合并会让"重复识别"被粘成
「材质材质」，所以必须先去掉重复的）。

界面上给了三档（`#opt-split-mode`，**模型和本地 OCR 都生效**）：

| 档位 | 行为 | 什么时候用 |
|---|---|---|
| `smart` 智能合并（默认） | 同一行上**挨得近的**才并 | 常规；表格列不会被并掉 |
| `whole` 不切分 | 只要是**同一行**就并，无论隔多远 | 上游切得太碎、"一行被拆成几段分别排版"时 |
| `off` 保持原样 | 完全不合并 | 排查"是不是合并合错了" |

三档都保留另外三条护栏：不跨行（纵向重叠 < 55%）、不并高度差过大的（比值 > 1.8）、
不并大幅重叠的（那是重复识别，交给去重）。也就是说"不切分"只去掉**间距**这一个上限，
其余判据照旧 —— 否则一行会跟上一行、跟插画粘成一大块。

### 折行宽度：中文不能只按"原文墨迹有多宽"折行

这是「有的字小」的**另一半原因**，比碎块更普遍。

中文哪怕字数比英文少，每个字占一个**全角**宽：12 个汉字 = 12em，
而 12 个字母只有约 6.6em。照原文墨迹宽度折行，一句中文常被折成 2~4 行；
行数一多就撞上"可用高度"上限（邻居约束），只能一路缩字号。

实测（真实规格图 2600×1682，34 条）：

| | 平均字号 | 平均行数 |
|---|---|---|
| 旧（按墨迹宽折行） | 21.4px | 1.1 |
| 新（用到右侧干净空白） | **25.7px** | 1.1 |

19 条变大、15 条持平、**0 条变小**；仍然 ≤8px 的从若干条降到 1 条。

**但扩张必须看像素，不能只看邻居。** 邻居只保证"那里没有**文字**"，
不保证"那里什么都没有" —— 那里可能有表格线、边框、插画，中文横穿一根表格线
比字小更难看。所以 `PZOverlay.freeRightWidth` 从墨迹右缘开始逐列扫
（只扫该条所在的几行），一旦某列出现"与背景亮度明显不同"的像素就停下。
扩张上限是再放宽一倍原文墨迹宽（共 3 倍）。

### 覆盖矩形与裁剪：中文的尾巴不能被 clip 掉

"贴不全"的另一层含义是**译文被裁**：覆盖矩形既是擦除范围、也是绘制时的
裁剪矩形，它被邻居边界夹住；而排版如果按"邻居之间的总宽度"放宽，
中文就会画到覆盖矩形之外，尾部一两个字被裁掉。

三处修正：

1. 横向可用空间按**对齐锚点**算（左对齐用"到右邻居为止"，居中两边各摊一半，
   右对齐不受右侧限制），不再一律用"邻居之间的总宽度"。
2. 覆盖矩形装不下这行中文时，**按实际可用宽度重排一次**（关掉"轻微溢出"）——
   宁可字小一点、多折一行，也不让字消失。
3. 兜底把绘制起点夹进覆盖矩形，保证整段都画得出来。

覆盖统计里会记 `relayout`（有多少条走了重排）。
